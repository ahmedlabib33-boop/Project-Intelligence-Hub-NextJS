...
def _call_groq(system_prompt: str, user_prompt: str, max_output_tokens: int) -> tuple[dict[str, Any], str, int]:
    ...
    if result.status != "success":
        raise RuntimeError(result.error or result.answer)
    return json.loads(result.answer), result.model, result.latency_ms
...
def create_structured_completion(
    ...
    settings = load_ai_settings()
    groq_key_ready = bool(os.getenv("GROQ_API_KEY", "").strip())
    active_provider = "openai" if settings.enabled and settings.api_key else "groq" if groq_key_ready else "local"
    active_model = settings.model if active_provider == "openai" else os.getenv("GROQ_MODEL_PRIMARY", "groq") if active_provider == "groq" else settings.model
    ...
    if (not settings.enabled or not settings.api_key) and groq_key_ready:
        started = time.perf_counter()
        try:
            payload, groq_model, groq_latency = _call_groq(system_prompt, user_prompt, settings.max_output_tokens)
            validated = _validate_payload(payload, required_keys)
            if validated is None:
                raise RuntimeError("Groq response did not match the required schema keys.")
            latency_ms = groq_latency or int((time.perf_counter() - started) * 1000)
            groq_settings = replace(settings, model=groq_model, prompt_version=f"{settings.prompt_version}-groq")
            _write_run(db_path, cache_key, feature_name, groq_settings, "ok", input_hash, validated, "", latency_ms)
            return AIResult(status="ok", data=validated, source="groq", cache_key=cache_key, model=groq_model, latency_ms=latency_ms)
        except Exception as exc:
            latency_ms = int((time.perf_counter() - started) * 1000)
            error = f"{exc.__class__.__name__}: {exc}"
            groq_settings = replace(settings, model=os.getenv("GROQ_MODEL_PRIMARY", "groq"))
            _write_run(db_path, cache_key, feature_name, groq_settings, "error", input_hash, None, error, latency_ms)
            return AIResult(status="error", source="groq", error=error, cache_key=cache_key, model=groq_settings.model, latency_ms=latency_ms)
