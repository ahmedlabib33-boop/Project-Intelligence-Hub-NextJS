...
function InsertButton([string]$content){
    ...
    # Preferred insertion: directly after a simple Output Studio heading.
    $patterns=@(
    ...
    foreach($p in $patterns){
        ...
        if($m.Success){
            ...
            return [pscustomobject]@{
                Success=$true
                Content=$u
                Method="Inserted after Output Studio heading"
            }
        }
    }

    # Actual app fallback: insert immediately before the first report viewer.
    # The audit confirmed src\app\page.tsx contains iframe/object report rendering.
    $viewerPatterns=@(
        '(?m)^(?<indent>\s*)<iframe\b',
        '(?m)^(?<indent>\s*)<object\b',
        '(?m)^(?<indent>\s*)<embed\b'
    ...
    foreach($viewerPattern in $viewerPatterns){
        $viewerMatch=[regex]::Match(
            $content,
            $viewerPattern,
        ...
        if($viewerMatch.Success){
            $indent=$viewerMatch.Groups["indent"].Value
            $insertion=$indent+"<OutputStudioDownloadButton />`r`n"
            $updated=$content.Insert($viewerMatch.Index,$insertion)

            return [pscustomobject]@{
                Success=$true
                Content=$updated
                Method="Inserted immediately before the first report viewer"
            }
        }
    }

    # JSX-expression fallback for conditionally rendered viewers.
    $conditionalPatterns=@(
        '(?<viewer><iframe\b)',
        '(?<viewer><object\b)',
        '(?<viewer><embed\b)'
    ...
    foreach($conditionalPattern in $conditionalPatterns){
        $conditionalMatch=[regex]::Match(
            $content,
            $conditionalPattern,
            [Text.RegularExpressions.RegexOptions]::IgnoreCase
        )
        ...
        if($conditionalMatch.Success){
            $updated=$content.Insert(
                ...
                "<OutputStudioDownloadButton />`r`n          "
            )

            return [pscustomobject]@{
                Success=$true
                Content=$updated
                Method="Inserted before a conditionally rendered report viewer"
            }
        }
    }
    ...
    return [pscustomobject]@{
        Success=$false
        Content=$content
        Method="No report viewer insertion point found"
    }
}
...
$files=@(
    ...
    Where-Object {
        ...
        $_.Name -ne "OutputStudioDownloadButton.tsx"
    }
)
$output=@()
$viewers=@()
...
## Conclusion
$(if($output.Count-eq0){"No Output Studio source file was found."}elseif($output.Count-gt1){"Multiple real Output Studio page files were found. Review before applying."}elseif($output[0].HasDownload){"A download implementation already appears to exist."}else{"Exactly one Output Studio source file was found without a download control. Apply mode is eligible."})
"@
...
$preferredTarget = @(
    $output | Where-Object {
        $_.RelativePath -ieq "src\app\page.tsx"
    }
)

if($preferredTarget.Count -eq 1){
    $target = $preferredTarget[0].File
    $targetRel = $preferredTarget[0].RelativePath
}
...
elseif($output.Count -eq 0){
    throw "No Output Studio page file found."
}
else{
    throw "Multiple real Output Studio page files found. Apply stopped."
}
...
if(-not$result.Success){throw "No iframe, object, embed, or Output Studio heading insertion point was found. Backup: $backup"}
