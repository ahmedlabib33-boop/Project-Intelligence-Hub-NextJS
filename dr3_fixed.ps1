...
$appName = if ($package.name) { [string]$package.name } else { Split-Path $ProjectRoot -Leaf }
$appVersion = if ($package.version) { [string]$package.version } else { "Not declared" }
$appDescription = if ($package.description) { [string]$package.description } else { "Not declared" }
...
$exclude = '\\(\.git|node_modules|\.next|dist|build|coverage|\.turbo|\.vercel|out|__pycache__|\.cache|_DR3_App_Intelligence_Report[^\\]*)(\\|$)'
